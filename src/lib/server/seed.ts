...
async function seed() {
	...
	const volunteers = [
		{ username: 'raymond', firstName: 'Raymond', lastName: 'Liu' },
		{ username: 'mary', firstName: 'Mary', lastName: 'Liu' },
		{ username: 'zilin', firstName: 'Zilin', lastName: 'Liu' }
	];
	...
	const activities = ['Repair', 'Trophies', 'Sort Supplies', 'Set Up Pool', 'Cleanup', 'Concession Stand', 'Timing', 'Registration Desk'];
	for (const name of activities) {
		await db.insert(schema.activityTypes).values({ name }).onConflictDoNothing();
	}
	...
	const sampleEvents = [
		...
		},
		{
			...
			endTime: '13:00',
			location: 'Carlton Place Water Dragon Pool',
			description: 'Set up lanes, timing equipment, and seating for the first meet.',
			volunteersNeeded: 8,
			budget: '150.00',
			importance: 'high' as const
		},
	...
	const sampleAnnouncements = [
		{
			...
			content: 'We are excited to kick off another great year. Check the events page and sign up to volunteer!'
		},
		{
			title: 'Volunteer Hours Reminder',
			content: 'Full member families need 30 hours, tryout families need 4 hours. Donations of $10 count as 1 hour.'
		}
	];
	...
	const defaultSettings = [
		...
		{ key: 'donation_to_hour_rate', value: '10', label: 'Dollars per Hour (Donation Rate)' }
	];
