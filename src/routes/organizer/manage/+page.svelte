...
<div style="margin-bottom:32px;">
  ...
  <div class="card" style="margin-top:12px;">
    {#if form?.settingsSuccess}
      <div
        style="background:#d4edda;padding:8px 12px;border-radius:6px;margin-bottom:12px;"
      >
        <p style="color:#155724;font-size:0.9rem;">
          {t[$lang].settingsUpdated}
        </p>
      </div>
    {/if}
...
<section style="margin-bottom:32px;">
  ...
  <div class="grid-2">
    ...
    <div class="card">
      ...
      {#if form?.childSuccess}<div class="alert alert-success" role="status">
          {form.childSuccess}
        </div>{/if}
      {#if form?.childError}<p class="error" role="alert">
          {form.childError}
        </p>{/if}
      <form method="POST" action="?/addChild" use:enhance>
        <div class="grid-2">
          <div class="form-group">
            <label for="c_first">{t[$lang].firstNameLabel}</label><input
              id="c_first"
              name="firstName"
              type="text"
              required
          ...
          <div class="form-group">
            <label for="c_last">{t[$lang].lastNameLabel}</label><input
              id="c_last"
              name="lastName"
              type="text"
              required
        ...
        <div class="grid-2">
          ...
          <div class="form-group">
            ...
            <button
              type="button"
              style="background:none;border:none;color:var(--primary);cursor:pointer;font-size:0.8rem;padding:4px 0;text-align:left;box-shadow:none;min-height:auto;"
              onclick={() =>
                (showAddChildLevelDetails = !showAddChildLevelDetails)}
            >
              {showAddChildLevelDetails
                ? t[$lang].hideLevelDetails
                : t[$lang].moreLevelDetails}
            </button>
            {#if showAddChildLevelDetails}
              <div
                style="margin-top:8px;font-size:0.8rem;color:var(--text-light);max-height:240px;overflow-y:auto;"
              >
                {#each swimLevels as lvl}
...
<div class="grid-2">
  ...
  <div>
    ...
    <div class="card" style="margin-top:12px;">
      {#if form?.activityError}<p class="error" style="margin-bottom:8px;">
          {form.activityError}
        </p>{/if}
      <form
        method="POST"
      ...
      >
        <input
          name="name"
          type="text"
          placeholder={t[$lang].newActivityPlaceholder}
          required
          style="flex:1;"
        />
      ...
      {#each data.activities as activity}
        ...
        >
          ...
          <form method="POST" action="?/toggleActivity" use:enhance>
            ...
            <button
              type="submit"
              class="btn {activity.active ? 'btn-outline' : 'btn-accent'}"
              style="padding:4px 10px;font-size:0.8rem;"
              >{activity.active ? t[$lang].disable : t[$lang].enable}</button
            >
          </form>
        </div>
      {/each}
  ...
  <div>
    ...
    <div class="card" style="margin-top:12px;">
      {#if form?.manualSuccess}
        ...
        >
          <p style="color:#155724;font-size:0.9rem;">
            {t[$lang].entryAddedSuccess}
          </p>
        </div>
      {/if}
      {#if form?.manualError}<p class="error" style="margin-bottom:8px;">
          {form.manualError}
        </p>{/if}
      <form method="POST" action="?/addManualHours" use:enhance>
        <div class="form-group">
          ...
          <select id="userId" name="userId" required>
            ...
            {#each data.volunteers as p}<option value={p.id}
                >{p.firstName} {p.lastName} ({p.email})</option
              >{/each}
          </select>
        </div>
        <fieldset
          class="form-group"
          style="border:none;padding:0;margin-bottom:16px;"
        >
          <legend
            style="font-weight:600;font-size:0.9rem;margin-bottom:6px;color:var(--text);"
            >{t[$lang].type}</legend
          >
          <div
            style="display:flex;gap:8px;"
            role="radiogroup"
            aria-label={t[$lang].entryType}
          >
            <button
              type="button"
              role="radio"
              aria-checked={manualType === "volunteering"}
              class="btn {manualType === 'volunteering'
                ? 'btn-primary'
                : 'btn-outline'}"
              style="flex:1;text-align:center;"
              onclick={() => (manualType = "volunteering")}
              >{t[$lang].hours}</button
            >
            <button
              type="button"
              role="radio"
              aria-checked={manualType === "donation"}
              class="btn {manualType === 'donation'
                ? 'btn-primary'
                : 'btn-outline'}"
              style="flex:1;text-align:center;"
              onclick={() => (manualType = "donation")}
              >{t[$lang].donation}</button
            >
          </div>
          <input type="hidden" name="type" value={manualType} />
        </fieldset>
        <div class="form-group">
          <label for="date">{t[$lang].date}</label><input
            id="date"
            name="date"
            type="date"
            required
        ...
        <div class="form-group">
          <label for="notes">{t[$lang].notes}</label><input
            id="notes"
            name="notes"
            type="text"
          />
    ...
    <div class="card" style="margin-top:12px;">
      ...
      <a
        href="/organizer/manage/emailing"
        class="btn btn-accent"
        style="display:block;text-align:center;margin-bottom:12px;"
        >Go to Emailing</a
      >
      <a
        href="/organizer/manage/email_settings"
        class="btn btn-outline"
        style="display:block;text-align:center;">Email settings</a
      >
    </div>
  </div>
</div>
...
<div style="margin-top:32px;margin-bottom:32px;">
  ...
  <p style="color:var(--text-light);font-size:0.9rem;margin-bottom:12px;">
    Adds enough hours so all of a volunteer's linked children meet their
    requirement.
  </p>
  <div class="card" style="margin-top:12px;">
    {#if form?.markMetSuccess}
      ...
      >
        <p style="color:#155724;font-size:0.9rem;">
          Volunteer marked as meeting requirements.
        </p>
      </div>
    {/if}
    {#if form?.markMetError}<p class="error" style="margin-bottom:8px;">
        {form.markMetError}
      </p>{/if}
    <form
      method="POST"
    ...
    >
      ...
      <div class="form-group" style="min-width:160px;margin-bottom:0;">
        ...
        <select id="markAs" name="markAs">
          <option value="full_member">Annual Member</option>
      ...
      <button
        ...
        onclick={(e) => {
          if (
            !confirm(
              "This will add hours to meet the selected requirement. Continue?",
            )
          )
            e.preventDefault();
        }}>Mark as Met</button
  ...
  <div class="grid-2">
    ...
    <div>
      ...
      <div class="card" style="margin-top:12px;">
        ...
        >
          <div class="form-group">
            <label for="announcementTitle">{t[$lang].announcementTitle}</label
            ><input id="announcementTitle" name="title" type="text" required />
          </div>
          <div class="form-group">
            <label for="announcementContent"
              >{t[$lang].announcementContent}</label
            ><textarea id="announcementContent" name="content" rows="3" required
            ></textarea>
          </div>
          <button type="submit" class="btn btn-accent"
            >{t[$lang].postAnnouncement}</button
          >
        </form>
        {#each data.announcements as a}
          <div
            style="display:flex;justify-content:space-between;align-items:start;padding:8px 0;border-bottom:1px solid var(--border);"
          >
            <div>
    ...
    <div>
      ...
      <div class="card" style="margin-top:12px;">
        ...
        {#if form?.archiveSuccess}
          ...
          >
            <p style="color:#155724;font-size:0.9rem;">
              {t[$lang].seasonArchivedSuccess}
            </p>
          </div>
        {/if}
        {#if form?.archiveError}<p class="error" style="margin-bottom:8px;">
            {form.archiveError}
          </p>{/if}
        <form
          method="POST"
          ...
          use:enhance
          onsubmit={(e) => {
            if (!confirm(t[$lang].archiveConfirm)) e.preventDefault();
          }}
        >
          <div class="form-group">
            <label for="archiveLabel">{t[$lang].seasonLabel}</label><input
              id="archiveLabel"
              name="label"
              type="text"
              placeholder={t[$lang].seasonLabelPlaceholder}
              required
            />
          </div>
          <button type="submit" class="btn btn-danger" style="width:100%;"
            >{t[$lang].archiveReset}</button
          >
        </form>
          ...
          {#each data.archives as archive}
            ...
            >
              ...
              <span style="font-size:0.85rem;color:var(--text-light);"
                >{new Date(archive.archivedAt).toLocaleDateString()}</span
              >
            </div>
          {/each}
        {/if}
