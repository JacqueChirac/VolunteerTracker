...
{#if data.volunteers.length === 0}
	<div class="card"><p style="color:var(--text-light);">No volunteers found.</p></div>
{:else}
				...
				{#each volunteer.children as child}
					<div style="margin-top:8px;padding:8px 12px;background:var(--bg);border-radius:6px;">
						<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
							...
							<span style="font-size:0.8rem;color:var(--text-light);">{child.status === 'tryout' ? 'Tryout' : 'Full Member'}</span>
						</div>
