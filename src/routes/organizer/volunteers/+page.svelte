<!-- volunteer list — organizer view -->
<script lang="ts">
	import type { PageData } from './$types';
	import VolunteerChildGraph from '$lib/components/VolunteerChildGraph.svelte';
	import { lang } from '$lib/stores/lang';
	import { t } from '$lib/i18n';

	let { data }: { data: PageData } = $props();
	let search = $derived(data.search ?? '');
	let showGraph = $state(true);
</script>

<h1>{t[$lang].volunteersTitle}</h1>
<p style="color:var(--text-light);margin-bottom:16px;">{t[$lang].volunteersSubtitle}</p>

<button type="button" class="btn btn-outline" style="margin-bottom:16px;" onclick={() => (showGraph = !showGraph)}>
	{showGraph ? t[$lang].hideConnectionMap : t[$lang].showConnectionMap}
</button>

{#if showGraph}
	<VolunteerChildGraph volunteers={data.volunteers} />
{/if}

<form method="GET" style="margin-bottom:20px;display:flex;gap:8px;">
	<input type="text" name="search" placeholder={t[$lang].searchByNameEmail} bind:value={search} style="max-width:300px;" />
	<button type="submit" class="btn btn-primary">{t[$lang].search}</button>
	{#if search}<a href="/organizer/volunteers" class="btn btn-outline">{t[$lang].clear}</a>{/if}
</form>

{#if data.volunteers.length === 0}
	<div class="card"><p style="color:var(--text-light);">{t[$lang].noVolunteersFound}</p></div>
{:else}
	{#each data.volunteers as volunteer (volunteer.id)}
		<div class="card" style="margin-bottom:12px;">
			<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px;">
				<h3><a href="/organizer/volunteers/{volunteer.id}">{volunteer.firstName} {volunteer.lastName}</a></h3>
				<span style="font-size:0.85rem;color:var(--text-light);">{volunteer.email} &middot; {volunteer.totalHours} hrs</span>
			</div>
			{#if volunteer.children.length === 0}
				<p style="font-size:0.9rem;color:var(--text-light);">{t[$lang].noChildrenLinked}</p>
			{:else}
				{#each volunteer.children as child}
					<div style="margin-top:8px;padding:8px 12px;background:var(--bg);border-radius:6px;">
						<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
							<span style="font-weight:500;">{child.firstName} {child.lastName}</span>
							<span style="font-size:0.8rem;color:var(--text-light);">{child.status === 'tryout' ? t[$lang].tryoutStatus : t[$lang].fullMemberStatus}</span>
						</div>
						<div class="progress-bar" style="height:16px;">
							<div class="progress-bar-fill" style="width:{Math.min(100, (child.totalHours / child.requiredHours) * 100)}%;font-size:0.7rem;">
								{child.totalHours} / {child.requiredHours} hrs
							</div>
						</div>
					</div>
				{/each}
			{/if}
		</div>
	{/each}
{/if}
