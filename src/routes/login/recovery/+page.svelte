...
<div class="login-page">
  <div class="login-box card">
    <div style="display:flex;justify-content:flex-end;margin-bottom:8px;">
      <button
        onclick={() => lang.update((l) => (l === "en" ? "fr" : "en"))}
        class="lang-btn"
      >
        {$lang === "en" ? "FR" : "EN"}
      </button>
      ...
      use:enhance={() => {
        return async ({ result }) => {
          if (result.type === "success") {
            ...
            showToast(
              $lang === "en" ? "System error" : "Erreur système",
              "error",
            );
          }
      ...
      {#if step === 1}
        <p class="instruction">
          {$lang === "en"
            ...
            : "Entrez votre courriel pour recevoir un code de réinitialisation."}
        </p>
      ...
      <form
        ...
        use:enhance={() => {
          return async ({ result }) => {
            if (result.type === "success") {
              showToast(
                $lang === "en" ? "Code verified" : "Code vérifié",
                "success",
              );
            } else if (result.type === "failure") {
              if (result.status === 429) {
      ...
      <form
        ...
        use:enhance={() => {
          return async ({ result }) => {
            if (result.type === "success") {
              showToast(
                $lang === "en" ? "Resent successfully" : "Renvoyé avec succès",
                "success",
              );
            } else if (result.type === "failure") {
              const message =
